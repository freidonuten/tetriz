...
namespace net
{
    ...
    public:
        ...
        {
            return setsockopt(descriptor_, IPPROTO_TCP, TCP_NODELAY, &value, sizeof(value)) == 0;
        }
    ...
    namespace socket
    {
        ...
        template <typename Host>
        struct read_only : protected detail::socket_policy<Host>
        {
            [[nodiscard]]
            auto read() -> std::optional<std::vector<uint8_t>>
            {
                ...
                while ((received = ::recv(this->self().descriptor(), buffer.data(), buffer.size(), 0)) > 0)
        ...
        template <typename Host>
        struct write_only : protected detail::socket_policy<Host>
        {
            void write(std::span<const uint8_t> payload) const
            {
                if (static_cast<ssize_t>(payload.size()) != ::write(this->self().descriptor(), payload.data(), payload.size()))
                {
                    throw socket_io_error(std::format("Failed to write to socket {}", this->self().descriptor()));
                }
            }
        };
        ...
        template <typename Host>
        struct read_write
            : public read_only<Host>
            , public write_only<Host>
        {};
    ...
    using ServerSocket = Socket<socket::server, socket::auto_closeable, socket::read_write>;
    using ClientSocket = Socket<socket::client, socket::auto_closeable, socket::read_write>;
    using GenericSocket = Socket<socket::server, socket::client, socket::auto_closeable, socket::read_write>;
    using ConnectionWrapper = Socket<socket::client, socket::read_write>;
